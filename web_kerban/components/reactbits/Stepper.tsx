...
export default function Stepper({
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col items-center justify-center p-4" {...rest}>
      <div className={`mx-auto w-full max-w-md ${stepCircleContainerClassName}`}>
          ...
          <div className={`px-6 pb-6 ${footerClassName}`}>
            <div className={`mt-4 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
              ...
              <button
                ...
                className="rounded bg-emerald-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-emerald-700 transition-colors"
                {...nextButtonProps}
              >
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators = false }: StepIndicatorProps) {
  ...
  return (
    ...
    >
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'rgb(var(--muted))', color: 'rgb(var(--muted-foreground))' },
          active: { scale: 1.1, backgroundColor: '#059669', color: '#fff' },
          complete: { scale: 1, backgroundColor: '#059669', color: '#fff' }
        }}
